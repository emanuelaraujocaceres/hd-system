...
- process_sale_transaction(p_sale_id, ...) -> TABLE(success, message). Cardapio anon. Grants: authenticated, anon, service_role. REGRESSÃO 20260831 removeu o anon (REVOKE ALL + só auth/service) — re-concedido em 20260908_cardapio_anon_regrant.sql (NUNCA revogar anon aqui, exceção 0f).
...
- fechar_comanda(p_session_id uuid, p_payments jsonb, p_operator_name text) -> jsonb. FINALIZADOR idempotente de comanda (20260905, fix 20260908 órfã/bipolar): trava sessao FOR UPDATE, valida org+filial (is_superadmin/get_user_org_id/get_user_branch_id), marca todas as sales 'pending' da MESMA MESA+SESSAO como completed (+ payments_json/payment_method/operator_name) — inclui órfãs (customer_session_id IS NULL) e bipolares (table_id = sessão.table_id mas customer_session_id divergente, mesma org/filial) — e fecha a sessao (status='completed', closed_at). NAO re-baixa estoque nem mexe em tables.status/stock_movements (a baixa ja ocorreu no addSale via process_sale_transaction; re-baixar duplicaria). Grants: authenticated + service_role (NAO anon - regra 9). Fix cobre skol fantasma/Mesa 1 e halls mesa teste.

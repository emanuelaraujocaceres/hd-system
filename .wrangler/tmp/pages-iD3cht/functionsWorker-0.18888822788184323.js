...
var init_tslib_es6 = __esm({
  "../node_modules/tslib/tslib.es6.mjs"() {
    init_functionsRoutes_0_36522442177792713();
  ...
  try {
    ...
    if (!serviceKey) {
      return new Response(JSON.stringify({
        success: false,
        message: "SUPABASE_SERVICE_ROLE_KEY n\xE3o configurada no servidor."
      }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    const { createClient: createClient2 } = await Promise.resolve().then(() => (init_dist4(), dist_exports));
    const supabaseAdmin = createClient2(supabaseUrl, serviceKey, {
    ...
    if (!token) {
      return new Response(
        JSON.stringify({ success: false, message: "N\xE3o autenticado." }),
    ...
    if (authErr || !authData?.user) {
      return new Response(
        JSON.stringify({ success: false, message: "Sess\xE3o inv\xE1lida ou expirada." }),
        { status: 401, headers: { "Content-Type": "application/json" } }
      );
    }
    const { data: callerProfile } = await supabaseAdmin.from("system_users").select("superadmin, role, organization_id").eq("id", authData.user.id).maybeSingle();
    const isSuperadmin = callerProfile?.superadmin === true;
    const isAdmin = callerProfile?.role === "admin";
    if (!isSuperadmin && !isAdmin) {
      return new Response(
        JSON.stringify({ success: false, message: "Acesso negado: apenas superadmin ou admin." }),
    ...
    if (!name || !code || !organization_id) {
      return new Response(JSON.stringify({
        success: false,
        message: "name, code e organization_id s\xE3o obrigat\xF3rios."
      }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    if (!isSuperadmin && callerProfile?.organization_id !== organization_id) {
      return new Response(JSON.stringify({
        success: false,
        message: "Acesso negado: admin s\xF3 pode criar filial na pr\xF3pria organiza\xE7\xE3o."
      }), { status: 403, headers: { "Content-Type": "application/json" } });
    }
    const branchId = id && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(id)) ? id : crypto.randomUUID();
...
// api/admin/create-organization.ts
async function onRequestPost2(context) {
  ...
  try {
    ...
    const { data: callerProfile } = await supabaseAdmin.from("system_users").select("superadmin").eq("id", authData.user.id).maybeSingle();
    if (!callerProfile?.superadmin) {
      return new Response(
        JSON.stringify({ success: false, message: "Acesso negado: apenas superadmin." }),
        { status: 403, headers: { "Content-Type": "application/json" } }
      );
    }
    const body = await request.json().catch(() => ({}));
    ...
    if (!org_name || !admin_name || !admin_email) {
      return new Response(JSON.stringify({
        success: false,
        message: "org_name, admin_name e admin_email s\xE3o obrigat\xF3rios."
      }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    const { data: existing } = await supabaseAdmin.from("system_users").select("id").eq("email", String(admin_email).toLowerCase()).maybeSingle();
    if (existing) {
      return new Response(JSON.stringify({
        success: false,
        message: "Este e-mail j\xE1 est\xE1 cadastrado no sistema."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    const tempPassword = Math.random().toString(36).slice(2, 6).toUpperCase() + Math.random().toString(36).slice(2, 6) + Math.random().toString(10).slice(2, 5) + "@";
    ...
    if (createAuthErr) {
      ...
      if (msg.includes("already registered") || msg.includes("already exists")) {
        return new Response(JSON.stringify({
          success: false,
          message: "Este e-mail j\xE1 possui uma conta no sistema. Use outro e-mail."
        }), { status: 200, headers: { "Content-Type": "application/json" } });
      }
      return new Response(JSON.stringify({
        success: false,
    ...
    if (orgErr) {
      ...
      return new Response(JSON.stringify({
        success: false,
        message: `Erro ao criar organiza\xE7\xE3o: ${orgErr.message}`
      }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    const { error: branchErr } = await supabaseAdmin.from("store_branches").insert({
      id: branchId,
    ...
    if (userErr) {
      try {
        await supabaseAdmin.from("store_branches").delete().eq("id", branchId);
      } catch {
      }
      try {
        await supabaseAdmin.from("organizations").delete().eq("id", orgId);
...
var init_create_organization = __esm({
  "api/admin/create-organization.ts"() {
    init_functionsRoutes_0_36522442177792713();
...
async function onRequestPost3(context) {
  ...
  try {
    ...
    if (!name || !email || !organization_id) {
      return new Response(JSON.stringify({
        success: false,
        message: "name, email e organization_id s\xE3o obrigat\xF3rios."
      }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    if (!isSuperadmin && callerProfile?.organization_id !== organization_id) {
      return new Response(JSON.stringify({
        success: false,
        message: "Acesso negado: admin s\xF3 pode criar usu\xE1rio na pr\xF3pria organiza\xE7\xE3o."
      }), { status: 403, headers: { "Content-Type": "application/json" } });
    }
    const { data: existing } = await supabaseAdmin.from("system_users").select("id").eq("email", String(email).toLowerCase()).eq("organization_id", organization_id).maybeSingle();
    if (existing) {
      return new Response(JSON.stringify({
        success: false,
        message: "J\xE1 existe um usu\xE1rio com este e-mail nesta organiza\xE7\xE3o."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    const finalPassword = password || Math.random().toString(36).slice(2, 6).toUpperCase() + Math.random().toString(36).slice(2, 6) + Math.random().toString(10).slice(2, 5) + "@";
    const { data: authUser, error: createAuthErr } = await supabaseAdmin.auth.admin.createUser({
...
// api/admin/delete-organization.ts
async function onRequestPost4(context) {
  ...
  try {
    ...
    if (!organization_id) {
      return new Response(JSON.stringify({
        success: false,
        message: "organization_id \xE9 obrigat\xF3rio."
      }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    if (!confirm_name || typeof confirm_name !== "string") {
      return new Response(JSON.stringify({
        success: false,
        message: "Confirma\xE7\xE3o necess\xE1ria: digite o nome exato da organiza\xE7\xE3o."
      }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (organization_id === DEFAULT_ORG_ID) {
      return new Response(JSON.stringify({
        success: false,
        message: "A organiza\xE7\xE3o padr\xE3o do sistema (HD-System) n\xE3o pode ser exclu\xEDda."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    const { data: org, error: orgErr } = await supabaseAdmin.from("organizations").select("id, name").eq("id", organization_id).maybeSingle();
    if (orgErr) {
      return new Response(JSON.stringify({
        success: false,
        message: `Erro ao consultar organiza\xE7\xE3o: ${orgErr.message}`
      }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    if (!org) {
      return new Response(JSON.stringify({
        success: false,
        message: "Organiza\xE7\xE3o n\xE3o encontrada."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    if (String(confirm_name).trim().toLowerCase() !== String(org.name).trim().toLowerCase()) {
      return new Response(JSON.stringify({
        success: false,
        message: "O nome digitado n\xE3o confere com o nome da organiza\xE7\xE3o. Nada foi exclu\xEDdo."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    const { data: sysUsers } = await supabaseAdmin.from("system_users").select("id").eq("organization_id", organization_id);
    ...
    if (rpcErr) {
      return new Response(JSON.stringify({
        success: false,
        message: `Erro ao excluir organiza\xE7\xE3o: ${rpcErr.message}`
      }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (!result?.success) {
      return new Response(JSON.stringify({
        success: false,
        message: result?.message || "Falha ao excluir organiza\xE7\xE3o."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    const authIds = (sysUsers || []).map((u) => u.id);
    const authErrors = [];
...
var init_delete_organization = __esm({
  "api/admin/delete-organization.ts"() {
    init_functionsRoutes_0_36522442177792713();
...
// api/admin/set-organization-active.ts
async function onRequestPost5(context) {
  ...
  try {
    ...
    if (organization_id === DEFAULT_ORG_ID && !nextActive) {
      return new Response(JSON.stringify({
        success: false,
        message: "A organiza\xE7\xE3o padr\xE3o do sistema (HD-System) n\xE3o pode ser desativada."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    const { data: org, error: orgErr } = await supabaseAdmin.from("organizations").select("id").eq("id", organization_id).maybeSingle();
    if (orgErr) {
      return new Response(JSON.stringify({
        success: false,
        message: `Erro ao consultar organiza\xE7\xE3o: ${orgErr.message}`
    ...
    const { error: updateErr } = await supabaseAdmin.from("organizations").update({
      active: nextActive,
      subscription_expires_at: nextActive ? null : (/* @__PURE__ */ new Date()).toISOString()
    }).eq("id", organization_id);
    if (updateErr) {
      return new Response(JSON.stringify({
        success: false,
        message: `Erro ao atualizar organiza\xE7\xE3o: ${updateErr.message}`
      }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    return new Response(JSON.stringify({
      success: true,
      active: nextActive,
      message: nextActive ? "Organiza\xE7\xE3o reativada \u2014 acesso online restaurado. O app do cliente volta a sincronizar automaticamente." : "Organiza\xE7\xE3o desativada \u2014 acesso online cortado. O app do cliente continua funcionando localmente; nada foi apagado."
    }), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch (e) {
...
var init_set_organization_active = __esm({
  "api/admin/set-organization-active.ts"() {
    init_functionsRoutes_0_36522442177792713();
...
async function onRequestPost6(context) {
  ...
  try {
    ...
    const { data: callerProfile } = await supabaseAdmin.from("system_users").select("superadmin, role, organization_id").eq("id", callerId).maybeSingle();
    const isSuperadmin = callerProfile?.superadmin === true;
    const isAdmin = callerProfile?.role === "admin";
    if (!isSuperadmin && !isAdmin) {
      return new Response(
    ...
    if (!user_id) {
      return new Response(
        JSON.stringify({ success: false, message: "user_id \xE9 obrigat\xF3rio." }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
    const nextActive = active !== false;
    if (user_id === callerId) {
      return new Response(JSON.stringify({
        success: false,
        message: "Voc\xEA n\xE3o pode desativar a sua pr\xF3pria conta."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    const { data: target, error: targetErr } = await supabaseAdmin.from("system_users").select("id, superadmin, name, email, organization_id").eq("id", user_id).maybeSingle();
    if (targetErr) {
      return new Response(
        JSON.stringify({ success: false, message: `Erro ao consultar usu\xE1rio: ${targetErr.message}` }),
        { status: 500, headers: { "Content-Type": "application/json" } }
      );
    }
    if (!target) {
      return new Response(
        JSON.stringify({ success: false, message: "Usu\xE1rio n\xE3o encontrado." }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    }
    if (target.superadmin) {
      return new Response(JSON.stringify({
        success: false,
        message: "Contas de superadmin n\xE3o podem ser desativadas."
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    if (!isSuperadmin && callerProfile?.organization_id !== target.organization_id) {
      return new Response(JSON.stringify({
        success: false,
        message: "Acesso negado: admin s\xF3 pode alterar usu\xE1rios da pr\xF3pria organiza\xE7\xE3o."
      }), { status: 403, headers: { "Content-Type": "application/json" } });
    }
    const { error: updateErr } = await supabaseAdmin.from("system_users").update({ active: nextActive }).eq("id", user_id);
    if (updateErr) {
      return new Response(
        JSON.stringify({ success: false, message: `Erro ao atualizar usu\xE1rio: ${updateErr.message}` }),
        { status: 500, headers: { "Content-Type": "application/json" } }
      );
    }
    return new Response(JSON.stringify({
      success: true,
      active: nextActive,
      message: nextActive ? `Conta de ${target.name} reativada \u2014 acesso restaurado.` : `Conta de ${target.name} desativada \u2014 o aparelho dele ser\xE1 desconectado automaticamente quando ficar online.`
    }), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch (e) {
...
var init_set_user_active = __esm({
  "api/admin/set-user-active.ts"() {
    init_functionsRoutes_0_36522442177792713();
...
var init_middleware = __esm({
  "_middleware.ts"() {
    init_functionsRoutes_0_36522442177792713();
    ALLOWED_ORIGINS = [
      "https://hd-system.pages.dev",
      // produção Cloudflare Pages
      "https://hd-system.vercel.app",
      // produção Vercel (se aplicável)
      "http://localhost:3000",
      // dev local (Vite/Express)
      "http://localhost:5173",
      // dev local (Vite default)
      "http://127.0.0.1:3000",
      // dev local (variant)
      "http://127.0.0.1:5173"
      // dev local (variant)
...
// ../.wrangler/tmp/pages-iD3cht/functionsRoutes-0.36522442177792713.mjs
var routes;
var init_functionsRoutes_0_36522442177792713 = __esm({
  "../.wrangler/tmp/pages-iD3cht/functionsRoutes-0.36522442177792713.mjs"() {
    init_create_branch();
    init_create_organization();
    init_create_user();
    init_delete_organization();
    init_set_organization_active();
    init_set_user_active();
    init_health();
    init_middleware();
    routes = [
      {

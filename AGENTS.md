...
- **Módulo Comandas (PDV restrito do operador + cardápio):** a "comanda" é a tabela `customer_sessions`; os itens vivem em `sales` + `sale_items` agrupados por `customer_session_id` (NÃO existe tabela `comandas`). O operador gerencia pela `ComandaView` (lista de mesas → detalhe com busca + qty + adicionar/remover + rodapé fixo) e o cliente pede pelo cardápio (`PublicMenuView`, venda `status='pending'` por rodada). **Modelo HÍBRIDO de estoque (decisão do usuário 2026-09-05):** a baixa atômica acontece AO ADICIONAR o item — `adicionarItem` (`src/services/comandaService.ts`) cria venda `pending` via `storageService.addSale` → RPC `process_sale_transaction` (idem cardápio anon). **`orderSource` do operador é `'comanda'` — NUNCA `'cardapio_digital'`** (decisão 2026-09-05): o KDS/Pedidos (`KDSView` filtra `cardapio_digital`/`delivery`) e o celular do cliente (`PublicMenuView` poll) só mostram pedidos feitos via QR; item do operador é interno à ComandaView (agrupada por `tableId`) e o checkout fecha por `customer_session_id` (pega os dois fluxos da mesma mesa). O checkout é um **FINALIZADOR**: RPC `fechar_comanda` (migration `20260905_fechar_comanda.sql`) marca todas as vendas `pending` da sessão como `completed` (+ `payments_json`/`payment_method`), fecha a sessão (`status='completed'`, `closed_at`) e NADA mais. **PROIBIDO re-baixar estoque / inserir `stock_movements` / mexer em `cash_sessions` no checkout** (a baixa já ocorreu no addSale; o trigger `fn_log_stock_changes` já logou — rebaixar duplica débito e log). `removerItem` → `cancelSaleWithStockRestore` (restaura estoque normal/composto/fração server-side). **Mesa livre = ausência de sessão `active`** (ocupação NÃO é `tables.status`, que só tem `active`/`inactive` = mesa existe/habilitada). RPC `fechar_comanda`: SECURITY DEFINER, idempotente (já `completed` → `already_closed`), `FOR UPDATE` anti-corrida, valida org+filial via `is_superadmin()`/`get_user_org_id()`/`get_user_branch_id()` (regra 9), grants SOMENTE `authenticated` + `service_role` (nunca anon — regra 9). `PaymentModal` com a prop opcional `comandaMode` desvia a persistência do pagamento para o callback da comanda (fluxo PDV idêntico; sem a prop, o PDV normal fica intocado — zero regressão).

- **PIX isolado por filial — estrito, sem fallback global (Decisão 2026-09-07):** `pixConfigService.getEffectivePixKey(branchId)` NÃO usa mais `settings.pixKey` (fallback removido — commits `94cbfd3`, `5c464cd`); retorna `null` se a filial não tiver linha em `pix_config`. Cada filial tem 1 linha isolada (`storageService.savePixConfig(branchId, {chavePix, tipoChave, nomeTitular, ativo})`); `SettingsView.tsx` carrega/salva por `getSelectedBranchId()` e recarrega ao trocar de filial (label "Filial atual: NOME (id)" + hint "isolada (cada filial tem sua chave, mesmo com 1 filial não há global)"). `INITIAL_SETTINGS.pixKey = ''` (`src/data/mockData.ts`, antes CNPJ fake `12345678000190`). **NUNCA reintroduzir fallback global** — vazaria PIX/chave entre filiais/orgs. Ref: `src/services/pixConfigService.ts`, `src/components/Settings/SettingsView.tsx`.

- **autoPrintReceipt = false por padrão (Decisão 2026-09-07):** `INITIAL_SETTINGS.autoPrintReceipt` mudado de `true` → `false` (`src/data/mockData.ts`, commit `42affd1`). Novas filiais/dispositivos não imprimem recibo automaticamente; impressão só com opt-in explícito em Configurações > Impressora. **Não reativar `true` como default.**

- **Fluxo Mesa completo — banner topo + checkout com troco (Decisão 2026-09-07):** Cliente em `#/mesa/:qr_token` pede (`PublicMenuView` → venda `pending`, `orderSource='cardapio_digital'`) e solicita conta (`kitchenStatus='closing_request'` via RPC `solicitar_fechamento_comanda`, com `payments[0].cashGiven/needChange` se troco) → operador vê banner topo fixo `OrderAlertBanner` (`src/components/shared/OrderAlertBanner.tsx`, `App.tsx` logo abaixo de `NotificationBanner`, `z-40`, `storageService.subscribe` + polling 2s, `posAudio.chime`+`vibrate`). Banner mostra (1) amber "X pedidos pendentes · Mesa Y → Aceitar" → navega p/ Pedidos (`kds`), (2) indigo "Fechamento solicitado · Mesa Y (N comandas)" → `hd:open-comanda` + `comandas` + `detailSessionId` (some ao clicar `Ver Comanda`/`Aceitar` — `233b99b`, tempo real sem interagir — `38b06df`). `KDSView.tsx` filtra `cardapio_digital` excluindo `completed/cancelled`, botão `X` em TODOS os status (inclusive `closing_request`/`delivered`, borda rose, tooltip "remove da Comanda e do cardápio"), `Fechamento Solicitado` abre `PaymentModal` completo (não modal legado) com `cartItems` agregados por `customerSessionId` + `subtotal`, `initialMethod`/`initialCashGiven` do `payments[0]` do cliente, `comandaMode.onConfirmComanda` → `fecharComanda(sessionId)` ou `saveSale` fallback + `recalcAndSyncCaixa`. `ComandaView.tsx` também abre `PaymentModal` com `initialMethod`/`initialCashGiven` do `closing` sale da sessão e `handleFinalizeComanda` via `fecharComanda` (agrupa TODOS os `pending` da mesa, mesmo operador+cardápio). `PaymentModal.tsx` aceita props `initialMethod`/`initialCashGiven` com `useEffect` sync ao abrir. Cardápio ordenado A-Z, cards `h-40 contain`. Commits `554ed61`, `5985f4c`, `38b06df`, `8eae5df`, `90b77b6`, `9c4e45f`, `e10c8a9`, `8d65286`, `804d5ef`.
...
**Extensão (fiado pré-fix que ficou `paid/R$0` indevidamente):** contas a receber de fiado marcadas `paid`/`amount=0` por bug antigo (baseline errado no `updateReceivableFromPayments` pré-BUG-003) mas com saldo pendente REAL jamais eram corrigidas — o guard "não reabrir conta quitada" do `createReceivableFromSale` bloqueava a reabertura, e o KPI do Financeiro ficava menor que o FiadosView (ex.: R$270,50 vs R$274,50; conta de 13/08 Cliente teste). `createReceivableFromSale` DEVE reabrir conta `paid` quando `remaining > 0.01` (volta a `pending`, limpa `paidDate`, sincroniza). Conta realmente quitada (`remaining <= 0`) permanece `paid`; conta `cancelled` NUNCA reabre. Regressão: 3 testes em `storageService.branch.test.ts`.
...
**Estensão crítica — venda SEM customerId ("Cliente Não Identificado"):** `isCustomerCreditSettled` NÃO pode retornar `false` só porque `sale.customerId` está vazio. Antes, vendas sem cliente quitadas no FiadosView deixavam resíduo preso no KPI "Contas a Receber" (mesmo com Fiados zerado). A chave de agrupamento deve ser `sale.customerId || '__no_customer__'` (ESPA-LHANDO o FiadosView) tanto no filtro de vendas quanto no de `credit_payments` (`cp.customerId || '__no_customer__'`). Regra intacta: NUNCA zerar quando o grupo está parcialmente quitado. Regressão extra: `storageService.branch.test.ts` (1 teste — venda sem customerId quitada vira paid). Commit `9ea9e5c`.
**Poda de recebíveis ÓRFÃOS (resíduo que volta do cloud após apagar dados locais):** `backfillReceivablesFromSales` (roda no fim da hidratação) deve, além de processar vendas com fiado, PODAR contas `financial` do tipo `receivable`, `category='fiado'`, `status='pending'` que NÃO têm venda de fiado ativa por trás (venda apagada, `credit_account` removido, ou resíduo já gravado no banco). Regra segura: marcar `paid`/`amount=0` e `syncFinancialAccount` apenas quando `id` da conta não corresponde a nenhuma venda com `getFiadoAmount>0`. NUNCA podar fiado legítimo (que tem venda com credit_account ativa). Sem isso, o órfão persiste no cloud e volta mesmo limpando o localStorage (o `mergeBy` do merge usa `safeCloud` quando `hasLocalData=false`). Regressão: 2 testes em `storageService.branch.test.ts` (órfão zerado / fiado ativo preservado).

### BUG-036: cash_sessions bloqueado em produção — MAX() em coluna UUID (2026-09-05)
**Sintoma:** Caixa aberto em uma máquina não sincronizava. A DLQ (`movimentacoes_falhas`) acumulava `function max(uuid) does not exist` em TODA escrita de `cash_sessions` (58 falhas entre 04/09 e 05/09); cloud sem nenhuma sessão aberta (5 sessões, todas `closed`); o celular mostrava caixa FECHADO (fail-closed em `getActiveCaixaSession`) enquanto a outra máquina operava normal com o caixa aberto no localStorage. Sinais: sessão aberta com contadores reais presa no payload da DLQ (source `tryUpsert`, retry contínuo), `expected_balance` correto, vendas/estoque sincronizando normalmente (só `cash_sessions` bloqueado).
**Causa:** Função VIVA no banco de produção `prevent_multiple_cash_sessions()` (trigger `prevent_multiple_cash_sessions_trigger`, `BEFORE INSERT OR UPDATE` quando `status='open'`) usava `SELECT COUNT(*), MAX(id), MAX(operator_name) FROM cash_sessions WHERE ...` — **`MAX(id)` numa coluna UUID não existe no Postgres** (`function max(uuid) does not exist`) → toda escrita de sessão aberta falhava. A função NÃO estava no repo (só no banco).
**Regra (NUNCA reintroduzir):**
- **NUNCA usar funções agregadas (`MAX`/`MIN`) em colunas UUID** em triggers/funções do banco. Para apontar "o registro mais recente", usar `SELECT ... INTO ... ORDER BY <col> LIMIT 1`.
- Guard de "uma sessão aberta por filial/usuário": implementar com `SELECT ... INTO ... LIMIT 1` + `IF <var> IS NOT NULL THEN RAISE EXCEPTION`, nunca agregados sobre UUID (os valores só servem para a mensagem de erro).
- **Triggers/funções vivos em produção podem não existir no repo** — ao investigar erro de escrita no banco, SEMPRE consultar `pg_trigger` + `pg_get_functiondef` antes de assumir que o código do repo é o que roda.
- Fix aplicado em produção em 2026-09-05: `prevent_multiple_cash_sessions()` reescrita com `LIMIT 1` (aplicada via SQL Editor; backup = `pg_get_functiondef` da versão com `MAX`, revert = `CREATE OR REPLACE`). `prevent_duplicate_cash_sessions()` (sã — usa `COUNT(*)` + subselects `LIMIT 1`) NÃO foi alterada.
**Local:** banco de produção (função + trigger `prevent_multiple_cash_sessions` — **não estão no repo**); frontend: `syncCaixaSession` (`storageService.ts` ~1096, upsert que cai na DLQ), `getActiveCaixaSession` (~4212, fail-closed: sem sessão aberta no cloud → caixa fechado), `openNewCaixaSession` (~4378, adota sessão aberta existente no cloud).

### BUG-037: Sessão expirada → escritas vão como anon → 42501 RLS + DLQ crescendo (2026-09-05)
**Sintoma:** Após expiração da sessão Supabase (token JWT vencido), o app NÃO re-logava: `syncService.ensureSession()` validava sessão com `SELECT id FROM products` — policy anon de `products` é escopada pelo header `x-branch-id` (cardápio), então sem o header o SELECT voltava 0 linhas SEM erro → falsa positiva "sessão ok". O app então fazia upserts SEM sessão válida → políticas de escrita negavam (`42501`) → DLQ `movimentacoes_falhas` acumulava centenas de entradas (744 em 4 dias; padrinho 05/09: queda de 42501 em vendas/estoque após limpar storage da máquina principal). Dado real ficava preso na fila local, retentando sempre.
**Causa:** (1) Validação de sessão baseada em SELECT de tabela protegida por policy anon com header — falso positivo de sessão válida; (2) `tryUpsert`/`tryDelete`/`tryUpsertBatch` escreviam mesmo sem sessão; (3) erros `UNAUTH`/auth eram DROPADOS na fila (pendência perdida até re-login manual); (4) re-login falhava por JWT skew (`isTransientAuthError` errou em "JWT issued at future" por assinatura duplicada de `isConnectionError` — log de `runMigration` fez o `retry` com `isConnectionError` barrando a retry correta); (5) a fila era processada sem checar sessão; (6) DLQ sem dedup — mesma operação replantada a cada retry.
**Regra (NUNCA reintroduzir):**
- **Validar sessão via `supabase.rpc('get_my_profile')`** (SECURITY DEFINER, retorna JSON `{ id }` para autenticado e **NULL sem erro** para anon — testar o retorno, não o erro). NUNCA validar com SELECT em tabela protegida por policy anon/header (`products` com `x-branch-id` dá 0 linhas sem erro → falso positivo).
- **Escrita NUNCA vai à rede como anon (fora do cardápio)**: `tryUpsert`/`tryDelete`/`tryUpsertBatch` DEVM chamar `guardSessionForWrite(table)` e retornar o erro sintético `UNAUTH` (`{ code: 'UNAUTH', message: 'no valid supabase session' }`) sem tocar o Supabase. Tabelas do cardápio anon (`ANON_WRITE_TABLES` = footer_messages, media_devices, printers, tables, customer_sessions, digital_menu_config) são isentas — aparelho sem perfil local escreve via policies anon (exceção 0f).
- **`UNAUTH` ENFILEIRA** (não dropa): `processPendingQueue` faz hold quando `!hasSession && !isMenuAnonMode()` — pendências sobem após re-login; UNAUTH é atendido por `isUnauthError`.
- **DLQ com dedup**: antes do INSERT em `movimentacoes_falhas`, SELECT da mesma (table_name + record_id + status='pending' + `ilike` prefixo da mensagem `split('\n')[0].slice(0,300)` + opcional org/branch) `limit(1)` — duplicado pendente → NÃO replanta.
- Aviso ao usuário: sessão expirada + rede ok + usuário logado → toast via ponte global `notifyToast` (evento `hd:toast`, `Toast.tsx`), throttled 5 min (`sessionWarnedAtRef` no `App.tsx`) — `useToast()` LANÇA fora do `ToastProvider`, e o `App` é PAI do provider, então o health check usa `notifyToast`.
**Local:** `src/services/syncService.ts` (`ensureSession` ~983 usa `get_my_profile` + try/catch em `localStorage.getItem`; `tryUpsert` ~515 / `tryDelete` / `tryUpsertBatch` com `guardSessionForWrite`; `processPendingQueue` ~921 com hold; helpers `UNAUTH_ERROR`, `isMenuAnonMode`, `isUnauthError`, const `ANON_WRITE_TABLES`; DLQ com dedup ~570), `src/components/shared/Toast.tsx` (`notifyToast` + listener `hd:toast`), `src/App.tsx` (toast de sessão expirada no health check), `src/services/syncService.test.ts` (mock `vi.hoisted` de auth/rpc/localStorage + `makeChain()`, 14 testes). Commit `57d4f96`.

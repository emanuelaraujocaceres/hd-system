...
(async()=>{
  ...
  const {data,error}=await s.from('sales').insert({
    ...
    code:'CARD-TEST-001',
    operator_id:'cardapio_digital',
    operator_name:'Cliente (Cardapio Digital)',
    table_id:uuid(),
    customer_session_id:uuid(),
    items:[{product_id:uuid(),product_name:'Burger Teste',unit_price:15,quantity:2,total:30}],
    subtotal:30,discount:0,total:30,status:'pending',order_source:'cardapio_digital',kitchen_status:'pending'
  }).select().single();

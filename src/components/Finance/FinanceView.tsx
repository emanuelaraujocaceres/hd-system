  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-[#18181b] border border-slate-200 dark:border-[#27272a] shadow-sm">
          <span className="text-xs font-semibold text-slate-500 dark:text-[#71717a]">Lucro dos Produtos</span>
          <p className={`text-2xl font-black mt-1 ${financeSummary.productProfit >= 0 ? 'text-indigo-600 dark:text-indigo-400' : 'text-rose-600'}`}>
            R$ {financeSummary.productProfit.toFixed(2)}
          </p>
        </div>
      </div>
              ...
              filteredAccounts.map((acc) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Expanded details - todas as recorrências com scroll */}
      ...
      {/* SUB-TAB 2: resumo operacional — base apenas em vendas concluídas da filial */}
      {activeSubTab === 'dre' && (
        <div className="max-w-3xl mx-auto space-y-4">
          ...
          <button onClick={() => onNavigateTab('sales-history')} className="w-full text-left p-5 rounded-2xl bg-emerald-600 text-white shadow-sm hover:bg-emerald-700 transition-colors">
            <span className="text-xs font-bold opacity-85">Total vendido · {financeSummary.salesCount} venda(s)</span>
            <p className="text-3xl font-black mt-1">R$ {financeSummary.total.toFixed(2)}</p>
            <span className="text-xs underline opacity-85">Ver vendas realizadas</span>
          </button>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ['Dinheiro', financeSummary.cash, 'text-emerald-600'],
              ['PIX', financeSummary.pix, 'text-sky-600'],
              ['Cartão de crédito', financeSummary.creditCard, 'text-indigo-600'],
              ['Cartão de débito', financeSummary.debitCard, 'text-violet-600'],
            ].map(([label, value, color]) => (
              <div key={label as string} className="p-4 rounded-2xl bg-white dark:bg-[#18181b] border border-slate-200 dark:border-[#27272a] shadow-sm">
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">{label}</span>
                <p className={`text-lg font-black mt-1 ${color}`}>R$ {(value as number).toFixed(2)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/25">
              <span className="text-xs font-bold text-amber-700 dark:text-amber-300">Custo dos produtos vendidos</span>
              <p className="text-2xl font-black text-amber-700 dark:text-amber-300 mt-1">R$ {financeSummary.costOfGoodsSold.toFixed(2)}</p>
            </div>
            <div className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/25">
              <span className="text-xs font-bold text-emerald-700 dark:text-emerald-300">Lucro dos produtos vendidos</span>
              <p className="text-2xl font-black text-emerald-700 dark:text-emerald-300 mt-1">R$ {financeSummary.productProfit.toFixed(2)}</p>
              <p className="text-[11px] text-emerald-700/80 dark:text-emerald-300/80 mt-1">Vendas menos custo dos itens, sem despesas e impostos estimados.</p>
            </div>
          </div>
        </div>
      )}

      {/* DRE detalhado preservado temporariamente fora da interface; o resumo acima evita números estimados para operadores. */}
      {false && activeSubTab === 'dre' && (

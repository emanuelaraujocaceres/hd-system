    ...
    <div className="p-3 sm:p-4 md:p-6 max-w-6xl mx-auto space-y-4 sm:space-y-6">
      ...
      {activeSubTab === 'cardapio' && (
        <div className="space-y-6">
          ...
          {/* ✅ Cardápio Delivery - URL e QR Code */}
          <div className="p-6 rounded-2xl bg-white dark:bg-[#18181b] border border-slate-200 dark:border-[#27272a] space-y-5 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">Cardápio Delivery</h3>
                <p className="text-xs text-slate-500 dark:text-[#71717a]">URL e QR Code para pedidos de delivery (sem mesa)</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-[10px] text-slate-500 dark:text-[#71717a]">
                Compartilhe este link com clientes para pedidos de delivery. O cardápio exibe todos os produtos marcados como "Exibir no Cardápio Digital".
              </p>
            </div>

            {/* QR Code do Delivery */}
            <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-[#09090b] rounded-xl border border-slate-200 dark:border-[#27272a]">
              <div className="w-24 h-24 bg-white dark:bg-white rounded-lg p-2 flex items-center justify-center">
                <img 
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(`${window.location.origin}${window.location.pathname}#/delivery`)}`} 
                  alt="QR Code Delivery" 
                  className="w-full h-full"
                />
              </div>
              <div className="flex-1 space-y-1">
                <p className="text-xs font-bold text-slate-900 dark:text-white">QR Code Delivery</p>
                ...
                <button
                  onClick={() => {
                    ...
                    const printWindow = window.open('', '_blank');
                    if (printWindow) {
                      printWindow.document.write(`<html><head><title>QR Code Delivery</title></head><body style="display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;"><div style="text-align:center;"><img src="https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(link)}" /><p style="font-family:sans-serif;margin-top:20px;">Scan to order delivery</p></div></body></html>`);
                      printWindow.document.close();
                      printWindow.print();
                    }
                  }}
                ...
                >
                  <PrinterLucide className="w-3 h-3" />
                  Imprimir QR Code
                </button>
              </div>
            </div>
